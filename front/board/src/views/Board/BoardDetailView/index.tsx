...
export default function BoardDetailView() {
  ...
  const getBoard = () => {
    if (board) return;
    axios
      .get(GET_BOARD_URL(boardNumber as string))
  ...
  const getBoardResponseHandler = (response: AxiosResponse<any, any>) => {
    const { result, message, data } =
      response.data as ResponseDto<GetBoardResponseDto>;
    if (!result || !data) {
      alert(message);
      navigator("/");
      return;
  ...
  const likeResponseHandler = (response: AxiosResponse<any, any>) => {
    const { result, message, data } =
      response.data as ResponseDto<LikeResponseDto>;
    if (!result || !data) {
      alert(message);
      return;
    }
      ...
      <Box>
        {openComment && (
          <Box>
            ...
            <Divider />
            <Box
              sx={{ p: "20px 0px", display: "flex", justifyContent: "center" }}
            >
              <Pagination
                page={pageNumber}
